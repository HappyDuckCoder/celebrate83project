        ...
        {wishes.map((wish, index) => (
          ...
          >
            ...
            >
              ...
              {hoverIndex === index && isCreator && (
                <div
                  className="absolute top-2 right-1 rounded-full flex items-center justify-center text-black transition-all cursor-pointer"
                  onClick={() => onDelete(index)}
                >
                  <TiDeleteOutline size={16} className="text-black" />
                </div>
              )}

              {/* Lời chúc xuất hiện khi hover */}
              {(hoverIndex === index || visibleIndexes.includes(index)) && (
                <motion.div
